from typing import Iterable

import pytest
from sqlalchemy.engine import Engine

from nf_web.app import create_app, db as the_db
from nf_web.db_scripts.init_db import init_db, destroy_db

from nf_web.users.models import User, Job, Role, UsersRoles

the_app = create_app(dict(
    TESTING=True,  # Propagate exceptions
    LOGIN_DISABLED=False,  # Enable @register_required
    MAIL_SUPPRESS_SEND=True,  # Disable Flask-Mail send
    SERVER_NAME='localhost',  # Enable url_for() without request context
    SQLALCHEMY_DATABASE_URI='sqlite:///:memory:',  # In-memory SQLite DB
    WTF_CSRF_ENABLED=False,  # Disable CSRF form validation
))

# Setup an application context (since the tests run outside of the webserver context)
the_app.app_context().push()

# Create and populate roles and users tables
init_db(the_db, the_app)

JOBS_TABLE_CLEANUP = [
                        User.__tablename__,
                        Job.__tablename__,
                        Role.__tablename__,
                        UsersRoles.__tablename__
                     ]


def cleanup_tables(engine: Engine, tables: Iterable[str]=JOBS_TABLE_CLEANUP) -> None:
    # tables = list(tables)
    # with engine.begin() as conn:
    #     for t in tables:
    #         conn.execute(f"delete from {t}")
    destroy_db(engine, the_app)


@pytest.fixture(scope='session')
def app():
    """ Makes the 'app' parameter available to test functions. """
    return the_app


@pytest.fixture(scope='session')
def db():
    """ Makes the 'db' parameter available to test functions. """
    yield the_db
    cleanup_tables(the_db)


@pytest.fixture(scope='function')
def session(db, request):
    """Creates a new database session for a test."""
    connection = db.engine.connect()
    transaction = connection.begin()

    options = dict(bind=connection, binds={})
    session = db.create_scoped_session(options=options)

    db.session = session

    def teardown():
        transaction.rollback()
        connection.close()
        session.remove()

    request.addfinalizer(teardown)
    return session


@pytest.fixture(scope='session')
def client(app):
    return app.test_client()


